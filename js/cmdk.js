(function () {
  ...
  var DEFAULT_NAV_LINKS = [
    ...
    { href: '/colophon', label: 'Colophon', desc: 'How this site is built: tech stack, tools, gear', keys: 'uses stack tools' }
  ];
  ...
  var INDEX = [
    ...
    { type: 'Articles', title: 'District 65 Board Remains at Impasse Over Salem Replacement', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/news/2026/01/23/district-65-board-remains-at-impasse-over-salem-replacement/', keys: 'education' },
    { type: 'Articles', title: 'Finals Return with Familiar 10% Weight', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/news/2025/12/12/finals-return-with-familiar-10-weight/', keys: 'education' },
    ...
    { type: 'Articles', title: 'Kits on the Rise: Field Hockey and Swim & Dive Eye State Success', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/sports/2025/10/24/kits-on-the-rise-field-hockey-and-swim-dive-eye-state-success/', keys: 'sports swimming' },
    { type: 'Articles', title: 'Equity or Exclusion? District 65 Draws a Federal Eye', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/opinion/2025/09/19/equity-or-exclusion-district-65-draws-a-federal-eye/', keys: 'education' },
    ...
    { type: 'Articles', title: 'Contentious Races Decided in Evanston', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/news/2025/04/17/contentious-races-decided-in-evanston/', keys: 'elections' },
    { type: 'Articles', title: '$2.5 Million Donation Added to the Auditorium Renovations', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/news/2025/03/14/2-5-million-dollar-donation-added-to-the-auditorium-renovations/', keys: 'education' },
    { type: 'Articles', title: 'Evanston\'s Mayoral Race: Biss vs. Boarini', desc: 'The Evanstonian', url: 'https://www.evanstonian.net/news/2025/02/14/evanston-mayoral-race/', keys: 'elections' },
  ...
  function addRecent(url) {
    var recent = getRecent().filter(function (u) { return u !== url; });
    recent.unshift(url);
  ...
  function matchQueryWord(qw, entry) {
    ...
    for (var i = 0; i < entry._words.length; i++) {
      ...
      if (overlap >= minOverlap) {
        ...
        for (var t = 0; t < entry._titleWords.length; t++) {
          ...
          var ov = 0, lim = Math.min(qw.length, ttw.length);
          while (ov < lim && qw[ov] === ttw[ov]) ov++;
          if (ov >= minOverlap) { titleHit = true; break; }
        }
    ...
    if (bestDist <= maxDist) {
      ...
      for (var t = 0; t < entry._titleWords.length; t++) {
        ...
        if (editDist(qw, ttw) <= maxDist) { titleFuzzy = true; break; }
      }
        ...
        for (var r = 0; r < recent.length; r++) {
          for (var j = 0; j < INDEX.length; j++) {
            if (INDEX[j].url === recent[r]) {
              recentEntries.push({
                type: 'Recent', title: INDEX[j].title,
                desc: INDEX[j].desc, url: INDEX[j].url, keys: INDEX[j].keys
              });
              break;
            }
          }
        }
        var pages = INDEX.filter(function (e) { return e.type === 'Pages'; });
  ...
  function highlightTitle(title, query) {
    ...
    for (var w = 0; w < words.length; w++) {
      ...
      if (subPos !== -1) {
        for (var c = subPos; c < subPos + qw.length; c++) highlight[c] = true;
        continue;
      }
      ...
      while ((match = re.exec(titleLower)) !== null) {
        ...
        while (overlap < limit && qw[overlap] === tw[overlap]) overlap++;
        if (overlap >= minOverlap) {
          for (var c = match.index; c < match.index + tw.length; c++) highlight[c] = true;
          found = true;
          break;
        }
      }
      ...
      re = /[a-z0-9]+/g;
      var bestDist = 99, bestIdx = -1, bestLen = 0;
      while ((match = re.exec(titleLower)) !== null) {
        var tw = match[0];
        if (tw.length < 3 || Math.abs(qw.length - tw.length) > maxDist) continue;
        var d = editDist(qw, tw);
      ...
      if (bestDist <= maxDist && bestIdx !== -1) {
        for (var c = bestIdx; c < bestIdx + bestLen; c++) highlight[c] = true;
      }
    }
  ...
  function render(entries) {
    ...
    groups.forEach(function (g) {
      ...
      g.items.forEach(function (e) {
        ...
        html += '<a class="cmdk-item' + (idx === activeIndex ? ' cmdk-active' : '') + '"'
