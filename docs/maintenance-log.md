...
| **2026-04-27** | **Default dev port:** local default `PORT` in `server.js` is **2029** (was 1123); README, `.env.example`, and EADDRINUSE hint updated. Contact + local dev from earlier the same day: visible email `rmccomb@VoteHub.com`; `LOCAL_DEV_HOSTS`; startup log includes `http://localhost`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Secrets:** `CLOUD_PASSWORD`, optional `TRUST_LOCALHOST_AUTH` and `LOCAL_DEV_HOSTS` (dev-only hostnames from `/etc/hosts` treated as local for resume API bypass), `NODE_ENV`. Document in README env table; production values live in **Vercel** project settings. `**MYSITE_DATA_DIR` was removed\*\* with the race API (tests no longer need a temp data dir).
